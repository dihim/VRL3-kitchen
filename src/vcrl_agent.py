...
class RLEncoder(nn.Module):
   def __init__(self, obs_shape, model_name, device):
       ...
       self.repr_dim = output_dimension
...
class Critic(nn.Module):
   def __init__(self, repr_dim, action_shape, feature_dim, hidden_dim, num_critics=5):
       super().__init__()
...
class VRL3Agent:
   ...
   def update(self, replay_iter, online_replay_iter, online_replay_storage, step, stage, use_sensor):
       ...
       if (online_replay_storage.len() > 16):
           batch_online = next(online_replay_iter)

           print("Offline batch", batch.shape)
           print("Online batch", batch_online.shape)

           combined_batch = np.concatenate((batch, batch_online))
       ...
       if stage == 2:
           ...
           #BC
           metrics.update(self.update_actor_vrl3(obs_combined, action, stddev, bc_weight,
                                               self.pretanh_penalty, self.pretanh_threshold))
   ...
   def update_critic_vrl3(self, obs, action, reward, discount, next_obs, stddev, update_encoder, conservative_loss_weight):
       ...
       with torch.no_grad():
           ...
           target_Q1, target_Q2 = random.sample(self.critic_target.forward(next_obs, next_action), 2)
       ...
       Q1, Q2 =  random.sample(self.critic(obs, action), 2)
   ...
   def update_actor_vrl3(self, obs, action, stddev, bc_weight, pretanh_penalty, pretanh_threshold):
       ...
       """
       actor_loss_combined = actor_loss_bc
