...
class KitchenEnv:
   ...
   def __init__(self, env_name, test_image=False, cam_list=['side'],
       ...
       #kitchen_multitask_v0.KitchenTaskRelaxV1(task=['microwave', 'bottom burner', 'slide cabinet'], size=(64, 64), proprio=True)
       ...
       if env_feature_type == 'state':
           ...
           env = BasicFrankaEnv(env, cameras=cam_list,
           ...
           height = KITCHEN_DIM
           width = KITCHEN_DIM
           #CHANGED - latent_dim = height*width*len(cam_list)*num_frames
           latent_dim = height*width*num_frames
           # RRL class instance is environment wrapper...
           env = BasicFrankaEnv(env, cameras=cam_list,
               height=height, width=width, latent_dim=latent_dim, hybrid_state=True,
       ...
       if env_feature_type == 'pixels':
           self._obs_spec = specs.BoundedArray(shape=(number_channel, KITCHEN_DIM, KITCHEN_DIM), dtype='uint8', name='observation', minimum=0, maximum=255)
