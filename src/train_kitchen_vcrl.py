...
import platform
# TODO: Change to egl when operating in GPU ENV
...
class Workspace:
   ...
   def setup(self):
       ...
       env_type = 'adroit' if env_name in ('hammer-v0','door-v0','pen-v0','relocate-v0') else 'kitchen'
       ...
       if env_type == 'adroit':
           ...
           print("OBS Spec Adroit", self.train_env.observation_spec())
       ...
       self.replay_storage = ReplayBufferStorage(data_specs, self.work_dir / 'buffer', offline=True)
       ...
       self.online_replay_storage = ReplayBufferStorage(data_specs, self.work_dir / 'online_buffer', offline=False)

       self.online_replay_loader = make_replay_loader(
           self.work_dir / 'online_buffer', self.cfg.replay_buffer_size,
           self.cfg.batch_size, self.cfg.replay_buffer_num_workers,
           self.cfg.save_snapshot, self.cfg.nstep, self.cfg.discount, self.replay_buffer_fetch_every,
           is_adroit=True)
           
       self._online_replay_iter = None
   ...
   @property
   def online_replay_iter(self):
       if self._online_replay_iter is None:
           self._online_replay_iter = iter(self.online_replay_loader)
       return self._online_replay_iter
   ...
   def eval_adroit(self, force_number_episodes=None, do_log=False):
       if ENV_TYPE != 'kitchen':
    ...
    #    print("\n=== Stage 2 started ===")
       ...
       if stage2_n_update > 0:
           for i_stage2 in range(stage2_n_update):
               ...
               metrics = self.agent.update(self.replay_iter, self.online_replay_iter, self.online_replay_storage,i_stage2, stage=2, use_sensor=IS_ADROIT)
               #print("done update")
               if i_stage2 % self.cfg.stage2_eval_every_frames == 0:
                   ...
                   average_score, succ_rate = self.eval_adroit(force_number_episodes=self.cfg.stage2_num_eval_episodes,
                                                               do_log=True)
                   print('Stage 2 step %d, BC loss: %.2f, score: %.2f, succ rate: %.2f' %
                           (i_stage2, metrics['actor_loss_bc'], average_score, succ_rate))
    ...
    #    """========================================== STAGE 3 =========================================="""
       ...
       while train_until_step(self.global_step):
           ...
           if not seed_until_step(self.global_step):
               metrics = self.agent.update(self.replay_iter, self.replay_iter, self.online_replay_storage, self.global_step, stage=3, use_sensor=IS_ADROIT)
